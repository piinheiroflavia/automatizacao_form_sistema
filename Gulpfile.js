      ...
      gulpIf(
        ...
        exec(
          ...
          gulpIf(
            ...
            gulpIf(
              conf.fastDev,
              `sass  src/site.scss:${conf.distPath}/css/site.css src/scss:${conf.distPath}/vendor/css src/scss/rtl:${conf.distPath}/vendor/css/rtl src/scss/pages:${conf.distPath}/vendor/css/pages src/fonts:${conf.distPath}/vendor/fonts src/libs:${conf.distPath}/vendor/libs --no-source-map`
            )
          ),
          function (err) {
            cb(err);
...
const buildPluginCopyTask = function () {
  return merge(
    src('src/libs/@form-validation/umd/**/*.*').pipe(
      dest(path.join(conf.distPath, 'vendor/libs/@form-validation/umd/'))
    ),
